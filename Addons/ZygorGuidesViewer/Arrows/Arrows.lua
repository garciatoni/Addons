...
function ZygorGuidesViewer_ArrowSkin_Mixin:CreateFrame()
	if not self.frame then
		self.frame = CreateFrame("BUTTON","ZygorGuidesViewerPointerArrow_".. self.id,Pointer.ArrowFrameCtrl,"ZygorGuidesViewerFrame_Arrow_".. self.id .. "_Template")
...
function ZygorGuidesViewerFrame_Arrow_Mixin:OnDragStop()
	...
	ZGV.db.profile.frame_positions.ZygorGuidesViewerPointer_ArrowCtrl={ZygorGuidesViewerPointer_ArrowCtrl:GetPoint()}
end
...
function ZygorGuidesViewerFrame_Arrow_Mixin:SetupSprites()
	...
	for k,v in ipairs{self:GetRegions()} do  if v.GetTexture then v:SetTexture(v:GetTexture():gsub("$./",arrowskindir),false) end end
	for k,v in ipairs{self.arrow:GetRegions()} do  if v.GetTexture then v:SetTexture(v:GetTexture():gsub("$./",arrowskindir),false) end end
