...
function ZGV:GetQuestLeaderBoards(questindex,questID)
	...
	for g=1,numgoals do
		...
		if leaderboard then
			...
			ZGV:Debug("Quest %s claims %d goals but leaderboard (%s) says nil,%s,%s",C_QuestLog.GetTitleForLogIndex(questindex),numgoals,g,tostring(type),tostring(complete))
...
function ZGV:QuestTracking_CacheQuestLog(from)
	...
	for i = 1,numEntries do
		...
		if not info.isHeader and info.title and info.questID then
