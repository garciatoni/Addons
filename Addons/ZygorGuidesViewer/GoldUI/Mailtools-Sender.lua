...
-- GLOBAL EasyFork,UIDropDownFork_SetAnchor,UIDropDownFork_SetWidth,ZM_Sender_Frame
...
function Mailtools:AddItemToInventory(bag,slot)
	...
	if itemid then
		...
		local isbound --TODO: was this supposed to be 'boa'?
		if bop or isbound then return false end
...
function Mailtools:FillSenderList(prefix)
	local frame = self.MainFrame.Sender_Frame
