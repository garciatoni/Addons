...
function PetBattle.PetJournal.JournalIconSetup()
	...
	if _G["PetJournalEnhancedListScrollFrame"]~= nil then
		--prefix="PetJournalEnhancedListScrollFrame" --TODO: unused?
	end
...
function PetBattle:GetPetBreedBySlot(bag,slot)
	...
	local _,_,_,BattlePetId,BattlePetLevel,BattlePetRarity,BattlePetHP,BattlePetAtt,BattlePetSpeed,_,BattlePetName = string.find(GetContainerItemLink(bag, slot),"(.*)battlepet:(%d+):(%d+):(%d+):(%d+):(%d+):(%d+):(.*)%[(.*)%]")
	return PetBattle:GetPetBreedByStats(BattlePetId,BattlePetHP,BattlePetAtt,BattlePetSpeed,BattlePetRarity,BattlePetLevel)
end
