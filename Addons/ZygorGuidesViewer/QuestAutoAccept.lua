...
function ZGV:QuestAutoAccept_InGossip()
	...
	for qnum,questsInfo in ipairs(quests) do
		if self.CurrentStep then
			for i,goal in ipairs(self.CurrentStep.goals) do
				if goal.action=="accept" and goal.quest and goal.questid==questsInfo.questID and goal:GetStatus()=="incomplete" then
					self:Debug("&qauto Opening quest  in gossip")
					C_GossipInfo.SelectAvailableQuest(qnum)
					return true
				end
			end
		end
		if ZGV.db.profile.autoacceptturninall and not questsInfo.isTrivial then
			self:Debug("&qauto Opening any quest in gossip: #%d %s",qnum,questsInfo.title)
			C_GossipInfo.SelectAvailableQuest(qnum)
			return true
		end
	end
	...
	for qnum, questInfo in ipairs(quests) do
		for i,goal in ipairs(self.CurrentStep.goals) do
			if goal.action=="turnin" and goal.questid == questInfo.questID and goal:GetStatus()=="incomplete" then
				...
				C_GossipInfo.SelectActiveQuest(qnum)
				return true
			end
		end
		if ZGV.db.profile.autoacceptturninall then -- let's not be picky
